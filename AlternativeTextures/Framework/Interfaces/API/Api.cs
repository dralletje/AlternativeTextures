...
namespace AlternativeTextures.Framework.Interfaces.API
{
    ...
    {
        public void AddAlternativeTexture(
            AlternativeTextureModel model,
            string owner,
            Texture2D texture
        );
        public void AddAlternativeTexture(
    ...
    public class Api : IApi
    {
        ...
        )
        {
            if (String.IsNullOrEmpty(owner))
            {
                _framework.Monitor.Log(
                    $"Unable to add AlternativeTextureModel {model.GetNameWithSeason()}: Owner property is not set."
                );
                return;
            }
            ...
            {
                if (
                    (seasons.Count() == 0 && s > 0) || (seasons.Count() > 0 && s >= seasons.Count())
                )
                {
                    continue;
                }
                ...
                textureModel.ItemName =
                    textureModel.GetTextureType() == "Grass" ? "Grass" : textureModel.ItemName;
                ...
                if (textures.Count() > 1)
                {
                    ...
                    {
                        _framework.Monitor.Log(
                            $"Unable to add alternative texture for item {textureModel.ItemName} from {textureModel.TextureId}: Split textures (texture_1.png, texture_2.png, etc.) are not allowed for Decoration types (wallpapers / floors)!",
                            LogLevel.Warn
                        );
                        continue;
                    }
                ...
                else
                {
                    ...
                    {
                        _framework.Monitor.Log(
                            $"Unable to add alternative texture for {textureModel.ItemName} from {textureModel.TextureId}: The required image width is 256 for Decoration types (wallpapers / floors). Please correct the image's width manually.",
                            LogLevel.Warn
                        );
                        continue;
                    }
                    else if (textureModel.IsDecoration())
                    {
                        ...
                        !_framework.SplitVerticalTexturesToModel(
                            textureModel,
        ...
        public Texture2D GetTextureForObject(Object obj, out Rectangle sourceRect)
        {
            ...
            var textureVariation = int.Parse(
                obj.modData[ModDataKeys.ALTERNATIVE_TEXTURE_VARIATION]
            );
            if (
                textureVariation == -1
            ...
            var xTileOffset = obj.modData.ContainsKey(ModDataKeys.ALTERNATIVE_TEXTURE_SHEET_ID)
                ? obj.ParentSheetIndex
                    - int.Parse(obj.modData[ModDataKeys.ALTERNATIVE_TEXTURE_SHEET_ID])
                : 0;
            ...
            {
                xTileOffset = obj.modData.ContainsKey("AlternativeTextureCurrentFrame") is false
                    ? 0
                    : int.Parse(obj.modData["AlternativeTextureCurrentFrame"]);
            }
            sourceRect = new Rectangle(
        ...
        public void SetTextureForObject(Object obj)
        {
            ...
            PatchTemplate.AssignDefaultModData(
                obj,
                instanceSeasonName,
                true,
                obj.bigCraftable.Value
        ...
        )
        {
            ...
            if (AlternativeTextures.textureManager.DoesObjectHaveAlternativeTextureById(modelName))
            {
                ...
                obj.modData[ModDataKeys.ALTERNATIVE_TEXTURE_VARIATION] =
                    optionalVariation.ToString();
                obj.modData[ModDataKeys.ALTERNATIVE_TEXTURE_SEASON] = String.IsNullOrEmpty(
                    optionalSeason
                )
                    ? String.Empty
                    : optionalSeason;
        ...
        public void ClearTextureForObject(Object obj)
        {
            ...
            obj.modData[ModDataKeys.ALTERNATIVE_TEXTURE_NAME] =
                $"{AlternativeTextures.DEFAULT_OWNER}.{instanceName}";
