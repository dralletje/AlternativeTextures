    ...
    internal class BuildingPatch : PatchTemplate
    {
        ...
        internal void Apply(Harmony harmony)
        {
            ...
            harmony.Patch(
                ...
                prefix: new HarmonyMethod(GetType(), nameof(ResetTexturePrefix))
            );
            harmony.Patch(
        ...
        internal static bool DrawPrefix(Building __instance, float ___alpha, SpriteBatch b)
        {
            ...
            if (__instance is Stable || __instance.maxOccupants.Value == TRACTOR_GARAGE_ID)
            {
                ...
                b.Draw(
                    ...
                    __instance.color * ___alpha,
                    0f,
                    new Vector2(0f, __instance.texture.Value.Bounds.Height),
                    4f,
                    SpriteEffects.None,
                    (float)((__instance.tileY.Value + __instance.tilesHigh.Value - 1) * 64) / 10000f
                );
        ...
        internal static Texture2D GetBuildingTextureWithPaint(
            Building building,
            AlternativeTextureModel textureModel,
            int textureVariation,
            bool canBePaintedOverride = false
        )
        {
            ...
            var textureWidth =
                canReallyBePainted ? baseTexture.Width / 2
            ...
            if (canReallyBePainted)
            {
                ...
                building.paintedTexture = GetPaintedOverlay(
                    ...
                    texture2D,
                    paintedTexture2D,
                    building.netBuildingPaintColor.Value
                );
                if (building.paintedTexture != null)
                {
        ...
        )
        {
            ...
            if (!color.Color1Default.Value)
            {
                ...
                ApplyPaint(
                    color.Color1Hue.Value,
                    color.Color1Saturation.Value,
                    color.Color1Lightness.Value,
                    ref painted_pixels,
                    paint_indices[0]
                );
            }
            if (!color.Color2Default.Value)
            {
                ...
                ApplyPaint(
                    color.Color2Hue.Value,
                    color.Color2Saturation.Value,
                    color.Color2Lightness.Value,
                    ref painted_pixels,
                    paint_indices[1]
                );
            }
            if (!color.Color3Default.Value)
            {
                ...
                ApplyPaint(
                    color.Color3Hue.Value,
                    color.Color3Saturation.Value,
                    color.Color3Lightness.Value,
                    ref painted_pixels,
                    paint_indices[2]
                );
            }
