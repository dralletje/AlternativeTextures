...
namespace AlternativeTextures.Framework.Patches.SpecialObjects
{
    internal class ChestPatch : PatchTemplate
    {
        ...
        private static bool DrawPrefix(
            Chest __instance,
            int ___currentLidFrame,
            int ____shippingBinFrameCounter,
        ...
        )
        {
            if (__instance.modData.ContainsKey(ModDataKeys.ALTERNATIVE_TEXTURE_NAME))
            {
                ...
                {
                    draw_x = Utility.Lerp(
                        __instance.localKickStartTile.Value.X,
                        draw_x,
                        __instance.kickProgress
                    );
                    draw_y = Utility.Lerp(
                        __instance.localKickStartTile.Value.Y,
                        draw_y,
                        __instance.kickProgress
                    );
                }
                float base_sort_order =
                    Math.Max(0f, ((draw_y + 1f) * 64f - 24f) / 10000f) + draw_x * 1E-05f;
                if (__instance.localKickStartTile.HasValue)
                {
                    spriteBatch.Draw(
                        Game1.shadowTexture,
                        Game1.GlobalToLocal(
                            Game1.viewport,
                            new Vector2((draw_x + 0.5f) * 64f, (draw_y + 0.5f) * 64f)
                        ),
                ...
                if (
                    ...
                    && (__instance.ParentSheetIndex == 130 || __instance.ParentSheetIndex == 232)
                )
                {
                    spriteBatch.Draw(
                    ...
                    spriteBatch.Draw(
                        ...
                        new Rectangle(
                            (
                                (___currentLidFrame - __instance.ParentSheetIndex)
        ...
        private static bool DrawRecolorPrefix(
            Chest __instance,
            int ___currentLidFrame,
            SpriteBatch spriteBatch,
            int x,
