...
internal class WoodChipperPatch : PatchTemplate
{
    ...
    internal WoodChipperPatch(IMonitor modMonitor, IModHelper modHelper)
    ...
    private static bool DrawPrefix(
        Phone __instance,
        NetRef<Object> ___depositedItem,
        bool ____isAnimatingChip,
    ...
    )
    {
        if (__instance.modData.ContainsKey(ModDataKeys.ALTERNATIVE_TEXTURE_NAME))
        {
            ...
            spriteBatch.Draw(
                ...
                destination,
                new Rectangle(
                    __instance.readyForHarvest.Value ? 16 : 0,
                    textureOffset,
            ...
            if (__instance.shakeTimer > 0)
            {
                spriteBatch.Draw(
                    textureModel.GetTexture(textureVariation),
                    new Rectangle(destination.X, destination.Y + 4, destination.Width, 60),
                    new Rectangle(32, textureOffset, 16, 15),
                    Color.White * alpha,
                    0f,
                    Vector2.Zero,
                    SpriteEffects.None,
            ...
            if (__instance.heldObject.Value != null)
            {
                ...
                if (__instance.heldObject.Value is ColoredObject)
                {
                    spriteBatch.Draw(
                        ...
                        Game1.getSourceRectForStandardTileSheet(
                            Game1.objectSpriteSheet,
                            __instance.heldObject.Value.ParentSheetIndex + 1,
                            16,
                            16
                        ),
                        (__instance.heldObject.Value as ColoredObject).color.Value * 0.75f,
                        0f,
                        new Vector2(8f, 8f),
                        4f,
                        SpriteEffects.None,
