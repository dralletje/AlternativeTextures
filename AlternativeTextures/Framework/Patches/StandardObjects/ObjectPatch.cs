    ...
    internal class ObjectPatch : PatchTemplate
    {
        ...
        internal void Apply(Harmony harmony)
        {
            ...
            if (PatchTemplate.IsDGAUsed())
            {
                ...
                {
                    if (
                        Type.GetType("DynamicGameAssets.Game.CustomObject, DynamicGameAssets")
                            is Type dgaObjectType
                        && dgaObjectType != null
                    )
                    {
                        harmony.Patch(
        ...
        )
        {
            if (__instance.modData.ContainsKey(ModDataKeys.ALTERNATIVE_TEXTURE_NAME))
            {
                ...
                if (__instance.bigCraftable.Value)
                {
                    ...
                    {
                        draw_layer =
                            Math.Max(0f, (float)((y + 1) * 64 + 2) / 10000f) + (float)x / 1000000f;
                    }
                    ...
                    if (__instance.Name.Equals("Loom") && __instance.MinutesUntilReady > 0)
                    {
                        spriteBatch.Draw(
                            ...
                            Math.Max(
                                0f,
                                (float)((y + 1) * 64) / 10000f + 0.0001f + (float)x * 1E-05f
                            )
                        );
                    }
                    ...
                    if (__instance.ParentSheetIndex == 126 && __instance.Quality != 0)
                    {
                        spriteBatch.Draw(
                            ...
                            new Rectangle(
                                ...
                                (__instance.Quality - 1)
                                    * 20
                                    / FarmerRenderer.hatsTexture.Width
                                    * 20
                                    * 4,
                                20,
                                20
                    ...
                    !Game1.eventUp
                    || (Game1.CurrentEvent != null && !Game1.CurrentEvent.isTileWalkedOn(x, y))
                )
                {
                    if (__instance.ParentSheetIndex == 590)
                    {
                        ...
                        int artifactOffset = (
                            (
                                Game1.currentGameTime.TotalGameTime.TotalMilliseconds % 1200.0
                                <= 400.0
                            )
                                ? (
                                    (int)(
                        ...
                        spriteBatch.Draw(
                            textureModel.GetTexture(textureVariation),
                            position2,
                            new Rectangle(artifactOffset, textureOffset, 16, 16),
                            color,
                            0f,
                            origin,
                ...
                float base_sort =
                    (float)((y + 1) * 64) / 10000f + __instance.TileLocation.X / 50000f;
                ...
                if (coloredObj != null)
                {
                    coloredObj.drawInMenu(
                        ...
                        Game1.GlobalToLocal(
                            Game1.viewport,
                            new Vector2(x * 64, (float)(y * 64) - 96f - 8f + yOffset)
                        ),
                        1f,
                        0.75f,
                        base_sort + 1.1E-05f
        ...
        )
        {
            ...
            if (placedObject is null)
            {
                ...
                {
                    flooring.modData[ModDataKeys.ALTERNATIVE_TEXTURE_SHEET_ID] =
                        __instance.ParentSheetIndex.ToString();

                    var flooringName =
                        $"{AlternativeTextureModel.TextureType.Flooring}_{GetFlooringName(flooring)}";
                    var flooringSeasonName =
                        $"{flooringName}_{Game1.GetSeasonForLocation(Game1.currentLocation)}";
                    if (
                ...
                var result =
                    Game1.random.Next(2) > 0
                        ? AssignModData(
                            ...
                            instanceSeasonName,
                            true,
                            placedObject.bigCraftable.Value
                        )
                        : AssignModData(
                            placedObject,
                            instanceName,
        ...
        private static void ObjectPostfix(
            Object __instance,
            string itemId,
            int initialStack,
            bool isRecipe = false,
        ...
        )
        {
            ...
            var instanceName =
                $"{AlternativeTextureModel.TextureType.ArtifactSpot}_{GetObjectName(__instance)}";
            var instanceSeasonName =
                $"{instanceName}_{Game1.GetSeasonForLocation(Game1.currentLocation)}";
