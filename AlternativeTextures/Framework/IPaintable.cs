...
readonly struct PaintableFromModData(ModDataDictionary modData) : IPaintable
{
    ...
    public DrawableTexture? PreviewTexture(TextureIdentifier textureIdentifier)
    {
        ...
        else
        {
            if (this.Related is StardewValley.Object Related)
            {
                var maybeTextureModel = AlternativeTextures.textureManager.GetSpecificTextureModel(textureIdentifier.Name);
                ...
                if (maybeTextureModel is { } textureModel)
                {
                    ...
                    {
                        Texture = texture,
                        SourceRect = SourceRects.GetSourceRectangle(textureModel, Related, textureModel.TextureWidth, textureModel.TextureHeight, variation),
                    };
                }
                else
                {
