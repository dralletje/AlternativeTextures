...
{
    public static ClippableSprite? PreviewTextureForFloor(UniqueTextureIdentifier textureIdentifier, WorldObject? maybeRelated)
    {
        var fullTexture = maybeRelated switch
        {
            WorldObject.TerrainFeature(Flooring flooring)
                ...
                Game1.content.Load<Texture2D>(data.Texture).Clip(new Rectangle(data.Corner.X, data.Corner.Y, 64, 64)),
            _ => new TextureSprite(AlternativeTextures.textureManager.GetTexture(textureIdentifier)!.Texture),
        };
            ...
            case { ForModel: { Type: TextureType.Decoration, String: "Wallpaper" } }:
                {
                    if (textureIdentifier.IsDefault)
                    {
                        ...
                        return Game1.content.Load<Texture2D>("Maps\\walls_and_floors").Clip(new Rectangle(which % 16 * 16, which / 16 * 48, 16, 48));
                    }
                    else
                    {
                        var textureModel = AlternativeTextures.textureManager.GetTexture(textureIdentifier);
            ...
            case { ForModel: { Type: TextureType.Decoration, String: "Floor" } }:
                {
                    ...
                    {
                        var which = textureIdentifier.Variation;
                        return Game1.content.Load<Texture2D>("Maps\\walls_and_floors").Clip(new Rectangle(which % 8 * 32, 336 + (which / 8 * 32), 32, 32));
                    }
                    else
                    {
                        var textureModel = AlternativeTextures.textureManager.GetTexture(textureIdentifier);
            ...
            case { ForModel: { Type: TextureType.Building } buildingModel, Variation: -1, Owner: var skinId }:
                {
                    ...
                    /// .DrawPosition ???
                    /// FRAMES????
                    List<ClippableSprite> layers =
                    [
                        texture.Clip(buildingData.SourceRect.NonZeroOrNull() ?? texture.Bounds),
                    .. buildingData
                    ...
                    return new ClippableSprite(layers);
                }
            ...
            default:
                {
                    ...
                    else
                    {
                        ...
                        {
                            WorldObject.Object(var @object) => textureModel.Texture.Clip(
                            ...
                            WorldObject.TerrainFeature(var terrainFeature) => terrainFeature switch
                            {
                                ...
                                Flooring flooring => textureModel.Texture.Clip(
                                ...
                                HoeDirt hoeDirt => textureModel.Texture.Clip(
                                ...
                                Grass grass => textureModel.Texture.Clip(
                                ...
                                Bush bush => textureModel.Texture.Clip(
                                    SourceRects.GetBushSourceRect(bush, textureModel.Variation, textureModel.TextureHeight)
                                ),
                                ResourceClump clump => textureModel.Texture.Clip(new Rectangle(0, 0, 32, 32)),
                                _ => null,
                            },
                            WorldObject.Wallpaper => null,
                            WorldObject.Floor => null,
                            WorldObject.Mailbox => null,
                            WorldObject.Building(var building) => textureModel.Texture.Clip(
...
static class TextureHelper
{
    ...
    public static ClippableSprite? GetDefault(ModelIdentifier modelIdentifier, WorldObject? related)
    {
        ...
        return related switch
        {
            ...
            {
                ({ Type: TextureType.Craftable }, { bigCraftable.Value: true }) => Game1.bigCraftableSpriteSheet.Clip(
                    StardewValley.Object.getSourceRectForBigCraftable(@object.ParentSheetIndex)
                ),

                ({ Type: TextureType.Craftable }, { bigCraftable.Value: false }) => Game1.objectSpriteSheet.Clip(
                ...
                (_, Furniture furniture) when ItemRegistry.GetData(furniture.QualifiedItemId) is { } data =>
                    data.GetTexture().Clip(furniture.sourceRect.Value),
                // new TextureSprite(data.GetTexture()),
            ...
            WorldObject.TerrainFeature(var terrainFeature) => terrainFeature switch
            {
                ...
                Flooring flooring => Function.Tap<ClippableSprite>(() =>
                {
                    ...
                    return texture.Clip(SourceRects.GetFlooringSourceRect(flooring, variation, 0));
                }),

                HoeDirt hoeDirt => Game1.cropSpriteSheet.Clip(
                    SourceRects.GetCropSourceRect(hoeDirt.crop, variation, 0)
                ),

                Grass grass => grass.texture.Value.Clip(SourceRects.GetGrassSourceRect(grass, variation, 0)),

                Bush bush => Bush.texture.Value.Clip(SourceRects.GetBushSourceRect(bush, variation, 0)),

                ResourceClump resourceclump => Function.Tap<ClippableSprite>(() =>
                {
                    ...
                    return texture.Clip(sourceRect);
                }),
