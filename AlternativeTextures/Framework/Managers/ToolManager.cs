...
namespace AlternativeTextures.Framework.Managers
{
    internal class ToolManager
    {
        ...
        public ToolManager(IModHelper helper)
        {
            ...
            // Setup toolNames
            toolKeyToData.Add(
                $"{AlternativeTextures.TOOL_TOKEN_HEADER}PaintBucket",
                Path.Combine(assetFolderPath, "PaintBucket.png")
            );
            toolKeyToData.Add(
                $"{AlternativeTextures.TOOL_TOKEN_HEADER}Scissors",
                Path.Combine(assetFolderPath, "Scissors.png")
            );
            toolKeyToData.Add(
                $"{AlternativeTextures.TOOL_TOKEN_HEADER}SprayCan",
                Path.Combine(assetFolderPath, "SprayCan.png")
            );
            toolKeyToData.Add(
                $"{AlternativeTextures.TOOL_TOKEN_HEADER}SprayCanRare",
                Path.Combine(assetFolderPath, "SprayCanRare.png")
            );
            toolKeyToData.Add(
                $"{AlternativeTextures.TOOL_TOKEN_HEADER}PaintBrush_Empty",
                Path.Combine(assetFolderPath, "PaintBrushEmpty.png")
            );
            toolKeyToData.Add(
                $"{AlternativeTextures.TOOL_TOKEN_HEADER}PaintBrush_Filled",
                Path.Combine(assetFolderPath, "PaintBrushFilled.png")
            );
            toolKeyToData.Add(
                $"{AlternativeTextures.TOOL_TOKEN_HEADER}Catalogue",
                Path.Combine(assetFolderPath, "Catalogue.png")
            );
        }
        ...
        internal void Edit_DataTools(IAssetData data)
        {
            ...
            AddToolDataEntry(
                ...
                "PaintBrush",
                "paint_brush",
                AlternativeTextures.PAINT_BRUSH_FLAG,
                null
            ).Texture = $"{AlternativeTextures.TOOL_TOKEN_HEADER}PaintBrush_Empty";
        }

        private ToolData AddToolDataEntry(
            IDictionary<string, ToolData> toolData,
            string toolKey,
        ...
        )
        {
            ...
            ToolData newToolData = new()
            {
                ...
                ModData = new() { [modDataKey] = modDataValue },
        ...
        {
            data.AsDictionary<string, ShopData>()
                .Data["Carpenter"]
                .Items.AddRange([
                    new() { Id = TOOL_ID_PAINT_BUCKET, ItemId = TOOL_ID_PAINT_BUCKET },
                    new() { Id = TOOL_ID_SCISSORS, ItemId = TOOL_ID_SCISSORS },
                    new() { Id = TOOL_ID_PAINT_BRUSH, ItemId = TOOL_ID_PAINT_BRUSH },
                    ...
                    new() { Id = TOOL_ID_CATALOGUE, ItemId = TOOL_ID_CATALOGUE },
                ]);
        }
