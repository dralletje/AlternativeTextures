...
internal class CatalogueMenu : IClickableMenu
{
    ...
    public CatalogueMenu(Farmer who)
        : base(0, 0, 900, 600, showUpperRightCloseButton: false)
    {
        // Set the owner
        _owner = who;
        ...
        foreach (var item in allFurniture.Where(f => f.Item is Furniture).Select(f => f.Item as Object))
        {
            ...
            var texturesAvailable = AlternativeTextures
                .textureManager.GetAvailableTextureModels(
                    itemId,
                    instanceName,
                    Game1.player.currentLocation.GetSeason()
                )
                .Sum(t => t.Variations);
            ...
            for (var c = 0; c < _texturesPerRow; c++)
            {
                ...
                _alternativeTextureButtons.Add(
                    new ClickableTextureComponent(
                        ...
                        availableModels.First().GetTexture(0),
                        new Rectangle(),
                        2f,
                        false
                    )
    ...
    public override void receiveLeftClick(int x, int y, bool playSound = false)
    {
        ...
        for (var k = 0; k < _objectButtons.Count; k++)
        {
            if (
                ...
                || _currentlyDisplayedObjects.Count == 0
                || _currentObjectIndex + k >= _currentlyDisplayedObjects.Count
                || !_objectButtons[k].containsPoint(x, y)
            )
            {
                continue;
            }
        ...
        for (var i = 0; i < _alternativeTextureButtons.Count; i++)
        {
            ...
            else
            {
                _paintBrushWarningText = AlternativeTextures.modHelper.Translation.Get(
                    "ui.labels.paint_brush.not_found"
                );
            }
    ...
    public override void performHoverAction(int x, int y)
    {
        ...
        foreach (var c in _alternativeTextureButtons)
        {
            if (c.containsPoint(x, y))
            {
                ...
                if (
                    c.item is not null
                    && c.item.modData.ContainsKey(ModDataKeys.ALTERNATIVE_TEXTURE_NAME)
                    && AlternativeTextures.textureManager.GetSpecificTextureModel(
                        c.item.modData[ModDataKeys.ALTERNATIVE_TEXTURE_NAME]
                    )
    ...
    public override void update(GameTime time)
    {
        ...
        // Adjust alphas
        _paintBrushWarningAlpha = _isDisplayingAlternativeTextures
            ? Math.Max(0f, _paintBrushWarningAlpha - 0.005f)
            : 0f;
                ...
                if (textureIndex < _currentlyDisplayedTextures.Count)
                {
                    ...
                    if (PatchTemplate.IsDGAUsed() && PatchTemplate.IsDGAObject(_selectedObject))
                    {
                        ...
                        b.Draw(
                            ...
                            _alternativeTextureButtons[i].scale,
                            SpriteEffects.None,
                            0.87f
                        );
                    }
                    else
                    {
                        _alternativeTextureButtons[i]
                            .item.drawInMenu(
                                b,
                                new Vector2(
                                    _alternativeTextureButtons[i].bounds.X,
                                    _alternativeTextureButtons[i].bounds.Y
                                ),
                                _alternativeTextureButtons[i].scale,
                                1f,
                                0.87f,
        ...
        else
        {
            ...
            for (var k = 0; k < _objectButtons.Count; k++)
            {
                ...
                IClickableMenu.drawTextureBox(
                    b,
                    purchaseTexture,
                    purchaseTextureRectangle,
                    _objectButtons[k].bounds.X,
                ...
                if (item.ShouldDrawIcon())
                {
                    ...
                    SpriteText.drawString(
                        b,
                        displayName,
                        _objectButtons[k].bounds.X + 96 + 8,
                        _objectButtons[k].bounds.Y + 28,
                        999999,
                        -1,
                        999999,
                        1f,
